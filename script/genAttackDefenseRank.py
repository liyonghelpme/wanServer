...
#uid total suc rank
def sortRank(name):
    sql = 'select * from %s where total > 0 order by suc desc' % (name)
    myCon.query(sql)
    ...
    for r in res:   
        ...
        try:
            ...
            arr.append({'uid':r['uid'], 'suc':r['suc'], 'total':r['total'], 'rank':rank, 'papayaId':userData['papayaId'], 'name':userData['name']})
