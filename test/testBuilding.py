...
r = base+'finishBuild/'+str(uid)+'/'+str(bid)+'/'+str(0)+'/10/10/0/1/1/%s' % (json.dumps({"gold":10}))
build = req(r)
...
r = base+'harvestPlant/'+str(uid)+'/'+str(bid)+'/'+json.dumps({'gold':10})
fin = req(r)
...
bid = bid+1
r = base+'finishBuild/%d/%d/%d/%d/%d/%d/1/1/%s' % (uid, bid, 224, 0, 0, 0, json.dumps({"gold":10}))
req(r)
